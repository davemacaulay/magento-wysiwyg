...
define([
    ...
    WysiwygWidget.Widget.prototype = {
        ...
        initialize: function(formEl, widgetEl, widgetOptionsEl, optionsSourceUrl, widgetTargetId) {
            $(formEl).insert({bottom: tinyMceWidgetTools.getDivHtml(widgetOptionsEl)});
            ...
            new Ajax.Request(this.optionsUrl,
                {
                    ...
                    onSuccess: function(transport) {
                        try {
                            tinyMceWidgetTools.onAjaxSuccess(transport);
                            this.switchOptionsContainer();
                            if ($(optionsContainerId) == undefined) {
                                this.widgetOptionsEl.insert({bottom: tinyMceWidgetTools.getDivHtml(optionsContainerId, transport.responseText)});
                            } else {
                                this.switchOptionsContainer(optionsContainerId);
                            }
                        } catch(e) {
                ...
                new Ajax.Request($(this.formEl).action,
                    {
                        ...
                        onComplete: function(transport) {
                            try {
                                tinyMceWidgetTools.onAjaxSuccess(transport);
                                tinyMceWidgetTools.dialogWindow.modal('closeModal');
            ...
            new Ajax.Request(this.chooserUrl,
                {
                    ...
                    onSuccess: function(transport) {
                        try {
                            tinyMceWidgetTools.onAjaxSuccess(transport);
                            this.dialogContent = tinyMceWidgetTools.getDivHtml(responseContainerId, transport.responseText);
                            this.openDialogWindow(this.dialogContent);
                        } catch(e) {
                            alert({
                                content: e.message
    ...
    window.tinyMceWidgetTools = tinyMceWidgetTools;
});
